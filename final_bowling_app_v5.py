...
if "round" not in st.session_state:
    ...
    # 🔀 今回のラウンドに使用されるチーム構成（スコア未入力でもチームだけ表示）
    if st.session_state.round >= 1:
        st.markdown(f"### 👥 ラウンド {st.session_state.round} のチーム構成（予定）")
        # ラウンド1ではプレラウンドのスコアを使用
        if st.session_state.round == 1 and st.session_state.history:
            prev_scores = st.session_state.history[0]["scores"]
        else:
            prev_scores = scores

        df = pd.DataFrame(list(prev_scores.items()), columns=["Name", "Score"])
    ...
    if st.session_state.round > 1 and st.session_state.history:
        ...
        st.markdown(f"### ⏪ ラウンド {prev['round']} の結果")
        for i, team in enumerate(prev["teams"]):
            members = [f"{n}（{s}）" for n, s in team]
            tag = "🏆 WINNER" if i == prev["winner"] else ""
